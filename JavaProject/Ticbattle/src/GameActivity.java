...
public class GameActivity extends JFrame implements Runnable{
    ...
    public GameActivity() throws SocketException {
        a=new ImageIcon("image/imga(2).png");
        b=new ImageIcon("image/imga(3).png");
        if(signal==1){
            serverPort=7777;
            clientPort=9999;
            outPort=8888;
    ...
    public void run() {//客户端
        while(true) {
            ...
            try {
                ...
                client.close();
                if(integer1==1){
                    JOptionPane.showConfirmDialog(null, "you lose", null, JOptionPane.CLOSED_OPTION);
                    boom=new int[9];
                    for(JButton jButton:buttons){
                        jButton.setIcon(null);
                        judge=2;
